const arcs1974 = [
  {
    startLat: 38.9072, // Washington, DC, USA
    startLng: -77.0369,
    endLat: 31.3285057,  // Nogales, Mexico
    endLng: -110.9450472
  },
  {
    startLat: 31.3285057, // Nogales, Mexico
    startLng: -110.9450472,
    endLat: 30.6268798,  // Magdalena de Kino, Mexico
    endLng: -110.9614533
  },
  {
    startLat: 30.6268798, // Magdalena de Kino, Mexico
    startLng: -110.9614533,
    endLat: 38.9072,  // Washington, DC, USA
    endLng: -77.0369
  },
  {
    startLat: 38.9072, // Washington, DC, USA
    startLng: -77.0369,
    endLat: 35.6768601,  // Tokyo, Japan
    endLng: 139.7638947
  },
  {
    startLat: 35.6768601, // Tokyo, Japan
    startLng: 139.7638947,
    endLat: 35.0115754,  // Kyoto, Japan
    endLng: 135.7681441
  },
  {
    startLat: 35.0115754, // Kyoto, Japan
    startLng: 135.7681441,
    endLat: 37.5666791,  // Seoul, South Korea
    endLng: 126.9782914
  },
  {
    startLat: 37.5666791, // Seoul, South Korea
    startLng: 126.9782914,
    endLat: 60.0,  // Vladivostok, Soviet Union
    endLng: 98.0
  },
  {
    startLat: 60.0, // Vladivostok, Soviet Union
    startLng: 98.0,
    endLat: 38.9072,  // Washington, DC, USA
    endLng: -77.0369
  },
  {
    startLat: 38.9072, // Washington, DC, USA
    startLng: -77.0369,
    endLat: 14.6027962,  // Fort-de-France, France
    endLng: -61.0676724
  },
  {
    startLat: 14.6027962, // Fort-de-France, France
    startLng: -61.0676724,
    endLat: 14.6113732,  // Martinique, France
    endLng: -60.9620777
  },
  {
    startLat: 14.6113732, // Martinique, France
    startLng: -60.9620777,
    endLat: 38.9072,  // Washington, DC, USA
    endLng: -77.0369
  },
];

const cities1974 = [
  { lat: 14.6113732, lng: -60.9620777, name: "Martinique" },
  { lat: 31.3285057, lng: -110.9450472, name: "Nogales" },
  { lat: 35.0115754, lng: 135.7681441, name: "Kyoto" },
  { lat: 30.6268798, lng: -110.9614533, name: "Magdalena de Kino" },
  { lat: 60.0, lng: 98.0, name: "Vladivostok" },
  { lat: 37.5666791, lng: 126.9782914, name: "Seoul" },
  { lat: 14.6027962, lng: -61.0676724, name: "Fort-de-France" },
  { lat: 35.6768601, lng: 139.7638947, name: "Tokyo" },
  { lat: 38.9072, lng: -77.0369, name: "Washington, DC" },
];

const arcs1975 = [
  {
    startLat: 38.9072, // Washington, DC, USA
    startLng: -77.0369,
    endLat: 50.8465573,  // Brussels, Belgium
    endLng: 4.351697
  },
  {
    startLat: 50.8465573, // Brussels, Belgium
    startLng: 4.351697,
    endLat: 40.4167047,  // Madrid, Spain
    endLng: -3.7035825
  },
  {
    startLat: 40.4167047, // Madrid, Spain
    startLng: -3.7035825,
    endLat: 47.7981346,  // Salzburg, Austria
    endLng: 13.0464806
  },
  {
    startLat: 47.7981346, // Salzburg, Austria
    startLng: 13.0464806,
    endLat: 41.8933203,  // Rome, Italy
    endLng: 12.4829321
  },
  {
    startLat: 41.8933203, // Rome, Italy
    startLng: 12.4829321,
    endLat: 41.9032459,  // Apostolic Palace, Vatican City
    endLng: 12.4551867
  },
  {
    startLat: 41.9032459, // Apostolic Palace, Vatican City
    startLng: 12.4551867,
    endLat: 38.9072,  // Washington, DC, USA
    endLng: -77.0369
  },
  {
    startLat: 38.9072, // Washington, DC, USA
    startLng: -77.0369,
    endLat: 50.7842863,  // Bonn, West Germany
    endLng: 7.2605602
  },
  {
    startLat: 50.7842863, // Bonn, West Germany
    startLng: 7.2605602,
    endLat: 50.56884,  // Linz am Rhein, West Germany
    endLng: 7.28445
  },
  {
    startLat: 50.56884, // Linz am Rhein, West Germany
    startLng: 7.28445,
    endLat: 52.2319581,  // Warsaw, Poland
    endLng: 21.0067249
  },
  {
    startLat: 52.2319581, // Warsaw, Poland
    startLng: 21.0067249,
    endLat: 50.0619474,  // Kraków, Poland
    endLng: 19.9368564
  },
  {
    startLat: 50.0619474, // Kraków, Poland
    startLng: 19.9368564,
    endLat: 60.1674881,  // Helsinki, Finland
    endLng: 24.9427473
  },
  {
    startLat: 60.1674881, // Helsinki, Finland
    startLng: 24.9427473,
    endLat: 44.4361414,  // Bucharest, Romania
    endLng: 26.1027202
  },
  {
    startLat: 44.4361414, // Bucharest, Romania
    startLng: 26.1027202,
    endLat: 45.3454687,  // Sinaia, Romania
    endLng: 25.5481055
  },
  {
    startLat: 45.3454687, // Sinaia, Romania
    startLng: 25.5481055,
    endLat: 44.7880163,  // Belgrade, Yugoslavia
    endLng: 20.4522489
  },
  {
    startLat: 44.7880163, // Belgrade, Yugoslavia
    startLng: 20.4522489,
    endLat: 38.9072,  // Washington, DC, USA
    endLng: -77.0369
  },
  {
    startLat: 38.9072, // Washington, DC, USA
    startLng: -77.0369,
    endLat: 48.6452851,  // Rambouillet, France
    endLng: 1.819207
  },
  {
    startLat: 48.6452851, // Rambouillet, France
    startLng: 1.819207,
    endLat: 38.9072,  // Washington, DC, USA
    endLng: -77.0369
  },
  {
    startLat: 38.9072, // Washington, DC, USA
    startLng: -77.0369,
    endLat: 40.190632,  // Peking, China
    endLng: 116.412144
  },
  {
    startLat: 40.190632, // Peking, China
    startLng: 116.412144,
    endLat: -6.1754049,  // Jakarta, Indonesia
    endLng: 106.827168
  },
  {
    startLat: -6.1754049, // Jakarta, Indonesia
    startLng: 106.827168,
    endLat: 14.5904492,  // Manila, Philippines
    endLng: 120.9803621
  },
  {
    startLat: 14.5904492, // Manila, Philippines
    startLng: 120.9803621,
    endLat: 38.9072,  // Washington, DC, USA
    endLng: -77.0369
  },
];

const cities1975 = [
  { lat: 50.8465573, lng: 4.351697, name: "Brussels" },
  { lat: 45.3454687, lng: 25.5481055, name: "Sinaia" },
  { lat: 44.7880163, lng: 20.4522489, name: "Belgrade" },
  { lat: 50.56884, lng: 7.28445, name: "Linz am Rhein" },
  { lat: 50.0619474, lng: 19.9368564, name: "Kraków" },
  { lat: 48.6452851, lng: 1.819207, name: "Rambouillet" },
  { lat: -6.1754049, lng: 106.827168, name: "Jakarta" },
  { lat: 52.2319581, lng: 21.0067249, name: "Warsaw" },
  { lat: 14.5904492, lng: 120.9803621, name: "Manila" },
  { lat: 41.8933203, lng: 12.4829321, name: "Rome" },
  { lat: 47.7981346, lng: 13.0464806, name: "Salzburg" },
  { lat: 50.7842863, lng: 7.2605602, name: "Bonn" },
  { lat: 41.9032459, lng: 12.4551867, name: "Apostolic Palace" },
  { lat: 44.4361414, lng: 26.1027202, name: "Bucharest" },
  { lat: 40.190632, lng: 116.412144, name: "Peking" },
  { lat: 60.1674881, lng: 24.9427473, name: "Helsinki" },
  { lat: 38.9072, lng: -77.0369, name: "Washington, DC" },
  { lat: 40.4167047, lng: -3.7035825, name: "Madrid" },
];

export const labelsDefaultCities = [...cities1974, ...cities1975];

export const allArcs = [];

export const datasets = {
  "All Cities": {
    labels: labelsDefaultCities
  },
  "1974 Trips": {
    arcs: arcs1974,
    labels: cities1974
  },
  "1975 Trips": {
    arcs: arcs1975,
    labels: cities1975
  },
};
