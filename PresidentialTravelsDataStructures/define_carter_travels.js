const arcs1977 = [
  {
    startLat: 38.9072, // Washington, DC, USA
    startLng: -77.0369,
    endLat: 51.5074456,  // London, United Kingdom
    endLng: -0.1277653
  },
  {
    startLat: 51.5074456, // London, United Kingdom
    startLng: -0.1277653,
    endLat: 54.9738474,  // Newcastle, United Kingdom
    endLng: -1.6131572
  },
  {
    startLat: 54.9738474, // Newcastle, United Kingdom
    startLng: -1.6131572,
    endLat: 46.2017559,  // Geneva, Switzerland
    endLng: 6.1466014
  },
  {
    startLat: 46.2017559, // Geneva, Switzerland
    startLng: 6.1466014,
    endLat: 38.9072,  // Washington, DC, USA
    endLng: -77.0369
  },
  {
    startLat: 38.9072, // Washington, DC, USA
    startLng: -77.0369,
    endLat: 52.22977,  // Warsaw, Polish People's Republic
    endLng: 21.01178
  },
  {
    startLat: 52.22977, // Warsaw, Polish People's Republic
    startLng: 21.01178,
    endLat: 35.689198,  // Tehran, Imperial State of Iran
    endLng: 51.388973
  },
  {
    startLat: 35.689198, // Tehran, Imperial State of Iran
    startLng: 51.388973,
    endLat: 38.9072,  // Washington, DC, USA
    endLng: -77.0369
  },
];

const cities1977 = [
  { lat: 54.9738474, lng: -1.6131572, name: "Newcastle" },
  { lat: 35.689198, lng: 51.388973, name: "Tehran" },
  { lat: 46.2017559, lng: 6.1466014, name: "Geneva" },
  { lat: 38.9072, lng: -77.0369, name: "Washington, DC" },
  { lat: 52.22977, lng: 21.01178, name: "Warsaw" },
  { lat: 51.5074456, lng: -0.1277653, name: "London" },
];

const arcs1978 = [
  {
    startLat: 38.9072, // Washington, DC, USA
    startLng: -77.0369,
    endLat: 28.6430858,  // New Delhi, India
    endLng: 77.2192671
  },
  {
    startLat: 28.6430858, // New Delhi, India
    startLng: 77.2192671,
    endLat: 26.30473,  // Daulatpur Nasirabad[4], India
    endLng: 74.73364
  },
  {
    startLat: 26.30473, // Daulatpur Nasirabad[4], India
    startLng: 74.73364,
    endLat: 24.638916,  // Riyadh, Saudi Arabia
    endLng: 46.7160104
  },
  {
    startLat: 24.638916, // Riyadh, Saudi Arabia
    startLng: 46.7160104,
    endLat: 24.091071,  // Aswan, Egypt
    endLng: 32.897306
  },
  {
    startLat: 24.091071, // Aswan, Egypt
    startLng: 32.897306,
    endLat: 48.8588897,  // Paris, France
    endLng: 2.320041
  },
  {
    startLat: 48.8588897, // Paris, France
    startLng: 2.320041,
    endLat: 49.0677708,  // Normandy, France
    endLng: 0.3138532
  },
  {
    startLat: 49.0677708, // Normandy, France
    startLng: 0.3138532,
    endLat: 49.2764624,  // Bayeux, France
    endLng: -0.7024738
  },
  {
    startLat: 49.2764624, // Bayeux, France
    startLng: -0.7024738,
    endLat: 48.8035403,  // Versailles, France
    endLng: 2.1266886
  },
  {
    startLat: 48.8035403, // Versailles, France
    startLng: 2.1266886,
    endLat: 50.8465573,  // Brussels, Belgium
    endLng: 4.351697
  },
  {
    startLat: 50.8465573, // Brussels, Belgium
    startLng: 4.351697,
    endLat: 38.9072,  // Washington, DC, USA
    endLng: -77.0369
  },
  {
    startLat: 38.9072, // Washington, DC, USA
    startLng: -77.0369,
    endLat: 10.5060934,  // Caracas, Venezuela
    endLng: -66.9146008
  },
  {
    startLat: 10.5060934, // Caracas, Venezuela
    startLng: -66.9146008,
    endLat: -15.7975717,  // Brasília
    endLng: -47.8951985
  },
  {
    startLat: -15.7975717, // Brasília
    startLng: -47.8951985,
    endLat: -22.906847,  // Rio de Janeiro, Brazil
    endLng: -43.172897
  },
  {
    startLat: -22.906847, // Rio de Janeiro, Brazil
    startLng: -43.172897,
    endLat: 6.4550575,  // Lagos, Nigeria
    endLng: 3.3941795
  },
  {
    startLat: 6.4550575, // Lagos, Nigeria
    startLng: 3.3941795,
    endLat: 6.328034,  // Monrovia, Liberia
    endLng: -10.797788
  },
  {
    startLat: 6.328034, // Monrovia, Liberia
    startLng: -10.797788,
    endLat: 38.9072,  // Washington, DC, USA
    endLng: -77.0369
  },
  {
    startLat: 38.9072, // Washington, DC, USA
    startLng: -77.0369,
    endLat: 8.9714493,  // Panama City, Panama
    endLng: -79.5341802
  },
  {
    startLat: 8.9714493, // Panama City, Panama
    startLng: -79.5341802,
    endLat: 38.9072,  // Washington, DC, USA
    endLng: -77.0369
  },
  {
    startLat: 38.9072, // Washington, DC, USA
    startLng: -77.0369,
    endLat: 50.7842863,  // Bonn, West Germany
    endLng: 7.2605602
  },
  {
    startLat: 50.7842863, // Bonn, West Germany
    startLng: 7.2605602,
    endLat: 50.05617,  // Wiesbaden-Erbenheim, West Germany
    endLng: 8.30283
  },
  {
    startLat: 50.05617, // Wiesbaden-Erbenheim, West Germany
    startLng: 8.30283,
    endLat: 50.1050925,  // Frankfurt, West Germany
    endLng: 8.5859703
  },
  {
    startLat: 50.1050925, // Frankfurt, West Germany
    startLng: 8.5859703,
    endLat: 52.5601976,  // West Berlin, West Germany
    endLng: 12.9823712
  },
  {
    startLat: 52.5601976, // West Berlin, West Germany
    startLng: 12.9823712,
    endLat: 50.7842863,  // Bonn, West Germany
    endLng: 7.2605602
  },
  {
    startLat: 50.7842863, // Bonn, West Germany
    startLng: 7.2605602,
    endLat: 38.9072,  // Washington, DC, USA
    endLng: -77.0369
  },
];

const cities1978 = [
  { lat: 50.1050925, lng: 8.5859703, name: "Frankfurt" },
  { lat: -22.906847, lng: -43.172897, name: "Rio de Janeiro" },
  { lat: -15.7975717, lng: -47.8951985, name: "Brasília" },
  { lat: 50.05617, lng: 8.30283, name: "Wiesbaden-Erbenheim" },
  { lat: 26.30473, lng: 74.73364, name: "Daulatpur Nasirabad" },
  { lat: 49.2764624, lng: -0.7024738, name: "Bayeux" },
  { lat: 48.8588897, lng: 2.320041, name: "Paris" },
  { lat: 6.4550575, lng: 3.3941795, name: "Lagos" },
  { lat: 50.7842863, lng: 7.2605602, name: "Bonn" },
  { lat: 50.8465573, lng: 4.351697, name: "Brussels" },
  { lat: 24.638916, lng: 46.7160104, name: "Riyadh" },
  { lat: 48.8035403, lng: 2.1266886, name: "Versailles" },
  { lat: 10.5060934, lng: -66.9146008, name: "Caracas" },
  { lat: 6.328034, lng: -10.797788, name: "Monrovia" },
  { lat: 28.6430858, lng: 77.2192671, name: "New Delhi" },
  { lat: 8.9714493, lng: -79.5341802, name: "Panama City" },
  { lat: 52.5601976, lng: 12.9823712, name: "West Berlin" },
  { lat: 24.091071, lng: 32.897306, name: "Aswan" },
  { lat: 49.0677708, lng: 0.3138532, name: "Normandy" },
  { lat: 38.9072, lng: -77.0369, name: "Washington, DC" },
];

const arcs1979 = [
  {
    startLat: 38.9072, // Washington, DC, USA
    startLng: -77.0369,
    endLat: 16.0000778,  // Basse-Terre, France
    endLng: -61.7333373
  },
  {
    startLat: 16.0000778, // Basse-Terre, France
    startLng: -61.7333373,
    endLat: 16.2528827,  // Guadeloupe, France
    endLng: -61.5686855
  },
  {
    startLat: 16.2528827, // Guadeloupe, France
    startLng: -61.5686855,
    endLat: 38.9072,  // Washington, DC, USA
    endLng: -77.0369
  },
  {
    startLat: 38.9072, // Washington, DC, USA
    startLng: -77.0369,
    endLat: 19.4326296,  // Mexico City, Mexico
    endLng: -99.1331785
  },
  {
    startLat: 19.4326296, // Mexico City, Mexico
    startLng: -99.1331785,
    endLat: 38.9072,  // Washington, DC, USA
    endLng: -77.0369
  },
  {
    startLat: 38.9072, // Washington, DC, USA
    startLng: -77.0369,
    endLat: 38.7647784,  // Lajes Field, Portugal
    endLng: -27.0863748
  },
  {
    startLat: 38.7647784, // Lajes Field, Portugal
    startLng: -27.0863748,
    endLat: 30.0443879,  // Cairo, Egypt
    endLng: 31.2357257
  },
  {
    startLat: 30.0443879, // Cairo, Egypt
    startLng: 31.2357257,
    endLat: 31.1991806,  // Alexandria, Egypt
    endLng: 29.8951716
  },
  {
    startLat: 31.1991806, // Alexandria, Egypt
    startLng: 29.8951716,
    endLat: 29.9870753,  // Giza, Egypt
    endLng: 31.2118063
  },
  {
    startLat: 29.9870753, // Giza, Egypt
    startLng: 31.2118063,
    endLat: 32.0852997,  // Tel Aviv, Israel
    endLng: 34.7818064
  },
  {
    startLat: 32.0852997, // Tel Aviv, Israel
    startLng: 34.7818064,
    endLat: 31.7788242,  // Jerusalem, Israel
    endLng: 35.2257626
  },
  {
    startLat: 31.7788242, // Jerusalem, Israel
    startLng: 35.2257626,
    endLat: 30.0443879,  // Cairo, Egypt
    endLng: 31.2357257
  },
  {
    startLat: 30.0443879, // Cairo, Egypt
    startLng: 31.2357257,
    endLat: 38.7647784,  // Lajes Field, Portugal
    endLng: -27.0863748
  },
  {
    startLat: 38.7647784, // Lajes Field, Portugal
    startLng: -27.0863748,
    endLat: 38.9072,  // Washington, DC, USA
    endLng: -77.0369
  },
  {
    startLat: 38.9072, // Washington, DC, USA
    startLng: -77.0369,
    endLat: 48.2083537,  // Vienna, Austria
    endLng: 16.3725042
  },
  {
    startLat: 48.2083537, // Vienna, Austria
    startLng: 16.3725042,
    endLat: 38.9072,  // Washington, DC, USA
    endLng: -77.0369
  },
  {
    startLat: 38.9072, // Washington, DC, USA
    startLng: -77.0369,
    endLat: 35.6768601,  // Tokyo, Japan
    endLng: 139.7638947
  },
  {
    startLat: 35.6768601, // Tokyo, Japan
    startLng: 139.7638947,
    endLat: 34.679545,  // Shimoda, Japan
    endLng: 138.945379
  },
  {
    startLat: 34.679545, // Shimoda, Japan
    startLng: 138.945379,
    endLat: 37.5666791,  // Seoul, South Korea
    endLng: 126.9782914
  },
  {
    startLat: 37.5666791, // Seoul, South Korea
    startLng: 126.9782914,
    endLat: 38.9072,  // Washington, DC, USA
    endLng: -77.0369
  },
];

const cities1979 = [
  { lat: 38.7647784, lng: -27.0863748, name: "Lajes Field" },
  { lat: 48.2083537, lng: 16.3725042, name: "Vienna" },
  { lat: 16.0000778, lng: -61.7333373, name: "Basse-Terre" },
  { lat: 35.6768601, lng: 139.7638947, name: "Tokyo" },
  { lat: 16.2528827, lng: -61.5686855, name: "Guadeloupe" },
  { lat: 30.0443879, lng: 31.2357257, name: "Cairo" },
  { lat: 32.0852997, lng: 34.7818064, name: "Tel Aviv" },
  { lat: 29.9870753, lng: 31.2118063, name: "Giza" },
  { lat: 38.9072, lng: -77.0369, name: "Washington, DC" },
  { lat: 31.7788242, lng: 35.2257626, name: "Jerusalem" },
  { lat: 37.5666791, lng: 126.9782914, name: "Seoul" },
  { lat: 31.1991806, lng: 29.8951716, name: "Alexandria" },
  { lat: 34.679545, lng: 138.945379, name: "Shimoda" },
  { lat: 19.4326296, lng: -99.1331785, name: "Mexico City" },
];

const arcs1980 = [
  {
    startLat: 38.9072, // Washington, DC, USA
    startLng: -77.0369,
    endLat: 41.8933203,  // Rome, Italy
    endLng: 12.4829321
  },
  {
    startLat: 41.8933203, // Rome, Italy
    startLng: 12.4829321,
    endLat: 45.4371908,  // Venice, Italy
    endLng: 12.3345898
  },
  {
    startLat: 45.4371908, // Venice, Italy
    startLng: 12.3345898,
    endLat: 41.9032459,  // Apostolic Palace, Vatican City
    endLng: 12.4551867
  },
  {
    startLat: 41.9032459, // Apostolic Palace, Vatican City
    startLng: 12.4551867,
    endLat: 44.7880163,  // Belgrade, Yugoslavia
    endLng: 20.4522489
  },
  {
    startLat: 44.7880163, // Belgrade, Yugoslavia
    startLng: 20.4522489,
    endLat: 40.4167047,  // Madrid, Spain
    endLng: -3.7035825
  },
  {
    startLat: 40.4167047, // Madrid, Spain
    startLng: -3.7035825,
    endLat: 38.7077507,  // Lisbon, Portugal
    endLng: -9.1365919
  },
  {
    startLat: 38.7077507, // Lisbon, Portugal
    startLng: -9.1365919,
    endLat: 38.9072,  // Washington, DC, USA
    endLng: -77.0369
  },
  {
    startLat: 38.9072, // Washington, DC, USA
    startLng: -77.0369,
    endLat: 35.6768601,  // Tokyo, Japan
    endLng: 139.7638947
  },
  {
    startLat: 35.6768601, // Tokyo, Japan
    startLng: 139.7638947,
    endLat: 38.9072,  // Washington, DC, USA
    endLng: -77.0369
  },
];

const cities1980 = [
  { lat: 40.4167047, lng: -3.7035825, name: "Madrid" },
  { lat: 35.6768601, lng: 139.7638947, name: "Tokyo" },
  { lat: 45.4371908, lng: 12.3345898, name: "Venice" },
  { lat: 41.9032459, lng: 12.4551867, name: "Apostolic Palace" },
  { lat: 41.8933203, lng: 12.4829321, name: "Rome" },
  { lat: 38.7077507, lng: -9.1365919, name: "Lisbon" },
  { lat: 38.9072, lng: -77.0369, name: "Washington, DC" },
  { lat: 44.7880163, lng: 20.4522489, name: "Belgrade" },
];

export const labelsDefaultCities = [...cities1977, ...cities1978, ...cities1979, ...cities1980];

export const allArcs = [];

export const datasets = {
  "All Cities": {
    labels: labelsDefaultCities
  },
  "1977 Trips": {
    arcs: arcs1977,
    labels: cities1977
  },
  "1978 Trips": {
    arcs: arcs1978,
    labels: cities1978
  },
  "1979 Trips": {
    arcs: arcs1979,
    labels: cities1979
  },
  "1980 Trips": {
    arcs: arcs1980,
    labels: cities1980
  },
};
